...
3. Run **`bin\run-all.bat`** on Windows, **`bin/run-all.sh`** on Linux and macOS.
...
bin/         the launchers: .bat for Windows, .sh for Linux and macOS
src/         the processor itself
...
No Docker, or Docker not running? `run-all` says so and carries on without it — you still get the passe-partout, and the sidecar holds the metadata line without a description and hashtags. To turn the LLM off for good, set `SIDECAR_ENABLED = False` in the script.

The individual steps live in `bin/` too, in both flavours, for when you need just one of them:

| | |
| --- | --- |
| `start-llm` | starts Ollama and downloads the model |
| `heic2jpeg-with-metadata` | converts `.heic`/`.heif` in `input/` to JPEG, keeping the metadata |
| `run-passepartout` | builds the passe-partouts and the sidecars |
...
`bin/heic2jpeg-with-metadata` is a pre-step for iPhone photos: it converts every `.heic`/`.heif` in `input/` to JPEG with FFmpeg, copies EXIF/XMP/IPTC/ICC across with ExifTool, and moves the `.heic` originals into `originals/`. The JPEG it produces stays in `input/` and is picked up by the next step.
...
```
set PASSEPARTOUT_MODEL=qwen2.5vl:3b        rem export ... on Linux
docker compose up -d
bin\run-all.bat
```
...
- **Python 3.9+** with **Pillow** — the launchers install Pillow automatically if it is missing. The LLM call uses the standard library, so there is nothing else to install.
...
- **FFmpeg** on PATH — only for the HEIC step.
...
3. Запустите **`bin\run-all.bat`** на Windows или **`bin/run-all.sh`** на Linux и macOS.

Это и есть весь процесс: скрипт поднимет локальную модель (при первом запуске скачается около 6 ГБ), сконвертирует `.heic`/`.heif`, если они найдутся, и соберёт паспарту. Понадобятся Python с Pillow и ExifTool в PATH — см. [Что нужно установить](#что-нужно-установить).
...
bin/         запускалки: .bat для Windows, .sh для Linux и macOS
src/         сам обработчик
...
Нет докера или он не запущен? `run-all` честно об этом скажет и продолжит без него: паспарту всё равно соберётся, а в `.txt` будет одна строка с метаданными, без описания и хештегов. Чтобы отключить модель насовсем, поставьте `SIDECAR_ENABLED = False` в скрипте.

Отдельные шаги тоже лежат в `bin/`, в обоих вариантах — на случай, когда нужен только один из них:

| | |
| --- | --- |
| `start-llm` | поднимает Ollama и скачивает модель |
| `heic2jpeg-with-metadata` | конвертирует `.heic`/`.heif` из `input/` в JPEG, сохраняя метаданные |
| `run-passepartout` | собирает паспарту и `.txt` |
...
`bin/heic2jpeg-with-metadata` — подготовительный шаг для фотографий с айфона: конвертирует все `.heic`/`.heif` из `input/` в JPEG через FFmpeg, переносит EXIF/XMP/IPTC/ICC с помощью ExifTool и убирает исходные `.heic` в `originals/`. Готовый JPEG остаётся в `input/` — его подхватит следующий шаг.
...
```
set PASSEPARTOUT_MODEL=qwen2.5vl:3b        rem на Linux — export ...
docker compose up -d
bin\run-all.bat
```
...
- **Python 3.9+** и **Pillow** — запускалки доставят Pillow сами, если его нет. Запрос к модели идёт на стандартной библиотеке, ставить больше нечего.
...
- **FFmpeg** в PATH — только для шага с HEIC.
